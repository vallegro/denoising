function [ res ] = KGI( y_noise, g_kernel, align, lambda_g)
...
for block_i = 1:align:im_size(1),
    for block_j = 1:align:im_size(2),
        ...
        lap = CalLap( graph );
        block = y_noise(block_i:block_i+align-1 , ...
                        block_j:block_j+align-1 );
        block_res = (eye(align*align)+lambda_g*lap)^(-1)*block(:);
        res(block_i:block_i+align-1 , block_j:block_j+align-1) = reshape(block_res,[align align]);                                        
